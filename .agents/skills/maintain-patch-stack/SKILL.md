...
3. Establish separate rewrite and product-report baselines before auditing:
   ...
   - For major product highlights, compare user docs and production entry points at the product-report base and target. Establish what users could do before and what they can do now; classify each highlight as a new capability, an enhancement, or a fix. Use commit history to find related work, then group its introduction and subsequent refinements into one feature-level account. A later refinement must not obscure that the entire capability is new in this upgrade.
...
9. Finish with a user-oriented upgrade report, not a commit-by-commit changelog:
   - **What's new from upstream:** Open with the stable release actually contained by `OLD_TIP`, the exact selected target and resolved commit, and whether the target was already present. Summarize the user-visible features and fixes in the product-report range, not merely the commits newly fetched or replayed. When targeting a release tag, state that newer `upstream/main` commits were deliberately excluded.
     Lead each major highlight with the overall capability and where users find it, followed by its useful refinements. For example, if the previous release only tracked historical token usage and the target adds live subscription quotas plus account pooling, report “New Usage → Limits tab for remaining subscription quotas and reset times, including pooling across accounts and environments.” Check that the wording does not imply the base release already had a newly introduced feature.
