...
Use `--model` and `--effort` for a one-run override when requested, for example:

```sh
t3 thread run "Play a Forest Shuffle trial" --model gpt-6-astra --effort ultra
```

These flags retain the project's provider instance and do not change its defaults. Report a CLI
validation error rather than retrying with another model or effort.
