...
const findReasoningDescriptor = (
  descriptors: ReadonlyArray<ProviderOptionDescriptor>,
): Extract<ProviderOptionDescriptor, { type: "select" }> | undefined =>
  descriptors.find(
    (descriptor): descriptor is Extract<ProviderOptionDescriptor, { type: "select" }> =>
      descriptor.type === "select" &&
      (REASONING_OPTION_IDS.has(descriptor.id) || descriptor.label.toLowerCase() === "reasoning"),
  );

const isSupportedOptionSelection = (
  descriptors: ReadonlyArray<ProviderOptionDescriptor>,
...
export const resolveThreadRunModelSelection = Effect.fn("resolveThreadRunModelSelection")(
  function* (input: {
    project: OrchestrationProjectShell;
    ...
    const inherited = input.project.defaultModelSelection ?? {
      instanceId: ProviderInstanceId.make("codex"),
      model: DEFAULT_MODEL,
    };
    if (input.model === undefined && input.effort === undefined) return inherited;

    const provider = input.providers.find(
      (candidate) => candidate.instanceId === inherited.instanceId,
    );
    if (!provider) {
      return yield* new ThreadCliError({
        operation: "validateModelSelection",
        detail: `Provider instance '${inherited.instanceId}' is not available.`,
      });
    }
    if (!provider.enabled || provider.availability === "unavailable") {
      return yield* new ThreadCliError({
        operation: "validateModelSelection",
        detail: `Provider instance '${inherited.instanceId}' is not enabled and available.`,
      });
    }

    const requestedModel = input.model ?? inherited.model;
    const model = resolveSelectableModel(provider.driver, requestedModel, provider.models);
    if (!model) {
      const available = provider.models.map((candidate) => candidate.slug).join(", ");
      return yield* new ThreadCliError({
        operation: "validateModelSelection",
        detail: `Model '${requestedModel}' is not supported by provider instance '${provider.instanceId}'.${available ? ` Available models: ${available}.` : ""}`,
      });
    }

    const selectedModel = provider.models.find((candidate) => candidate.slug === model)!;
    const descriptors = selectedModel.capabilities?.optionDescriptors ?? [];
    let options =
      input.model === undefined
        ? [...(inherited.options ?? [])]
        : (inherited.options ?? []).filter((selection) =>
            isSupportedOptionSelection(descriptors, selection),
          );

    if (input.effort !== undefined) {
      const descriptor = findReasoningDescriptor(descriptors);
      if (!descriptor) {
        return yield* new ThreadCliError({
          operation: "validateModelSelection",
          detail: `Model '${model}' does not support a reasoning-effort selection.`,
        });
      }
      const effort =
        descriptor.options.find((choice) => choice.id === input.effort)?.id ??
        descriptor.options.find(
          (choice) => choice.label.toLowerCase() === input.effort!.toLowerCase(),
        )?.id;
      if (!effort) {
        return yield* new ThreadCliError({
          operation: "validateModelSelection",
          detail: `Reasoning effort '${input.effort}' is not supported by model '${model}'. Available efforts: ${descriptor.options.map((choice) => choice.id).join(", ")}.`,
        });
      }
      options = [
        ...options.filter((selection) => selection.id !== descriptor.id),
      ...
      (session) =>
        Effect.gen(function* () {
          ...
          return yield* use({
            snapshot,
            getProviders: () =>
              timeout(client.orchestration.providers({ headers })).pipe(
                Effect.mapError(
                  (cause) =>
                    new ThreadCliError({
                      operation: "readProviders",
                      detail: "Could not read provider model capabilities from the running server.",
                      cause,
                    }),
                ),
              ),
...
const runCommand = Command.make("run", {
  ...
  model: Flag.string("model").pipe(
    Flag.withDescription("Model for this thread. Defaults to the project's model selection."),
    Flag.optional,
  ),
  effort: Flag.string("effort").pipe(
    Flag.withDescription("Reasoning effort for this thread's model."),
    Flag.optional,
  ),
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      return yield* runLive(
        flags,
        "operate",
        Effect.fn("threadRun")(function* ({ snapshot, getProviders, dispatch }) {
          const selector = Option.getOrUndefined(flags.project) ?? process.cwd();
          const project = yield* resolveThreadProject(snapshot, selector);
          const requestedModel = Option.getOrUndefined(flags.model);
          const requestedEffort = Option.getOrUndefined(flags.effort);
          const modelSelection = yield* resolveThreadRunModelSelection({
            project,
            providers:
