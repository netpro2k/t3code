...
import {
  ...
  parseMiseOutdatedLatestVersion,
...
const makeMiseMock = (
  ...
  options: {
    ...
    readonly outdated?: Record<string, { readonly latest: string }>;
    readonly ls: unknown;
  },
) => {
  ...
  const script = [
    ...
    `  echo '${JSON.stringify(options.outdated ?? {})}'; exit 0`,
    "fi",
    'if [ "$1" = "ls" ] && [ "$2" = "--json" ]; then',
...
const npmCapabilities = makeProviderMaintenanceCapabilities({
  provider: driver("packageTool"),
  packageName: "@example/package-tool",
  updateExecutable: "npm",
  updateArgs: ["install", "-g", "@example/package-tool@latest"],
  updateLockKey: "npm-global:/test/prefix",
});
const miseCapabilities = {
  ...npmCapabilities,
  update: {
    ...npmCapabilities.update!,
    executable: "mise",
    args: ["upgrade", "claude"],
    lockKey: "mise",
  },
...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it.effect("switches package-managed providers to mise upgrades when mise owns the binary", () =>
    Effect.gen(function* () {
      ...
      expect(capabilities).toEqual({
        ...
        managedLatest: {
          executable: "mise",
          args: ["outdated", "--json", "package-tool"],
          toolName: "package-tool",
        },
      });
    }),
  );
  ...
  it.effect("keeps npm updates for packages exposed through a mise-managed runtime", () =>
    Effect.gen(function* () {
      ...
      expect(capabilities.update).toMatchObject({
        executable: "npm",
        lockKey: `npm-global:${normalizeCommandPath(NodeFS.realpathSync(NodePath.join(installsDir, "node", "26.7.0")))}`,
      });
    }),
  );
  ...
  it.effect("resolves managed latest versions through the tool manager's own recency gate", () =>
    Effect.gen(function* () {
      ...
      const miseMockBinDir = makeMiseMock(tempDir, {
        which: {},
        outdated: { claude: { latest: "2.1.235" } },
        ls: {},
      });
      const previousPath = process.env.PATH;
      process.env.PATH = `${miseMockBinDir}:${previousPath ?? ""}`;
      try {
        const capabilities = miseCapabilities;
  ...
  it.effect("treats a mise tool omitted from outdated output as current", () =>
    parseMiseOutdatedLatestVersion({
      stdout: "{}",
      toolName: "claude",
      currentVersion: "2.1.234",
    }).pipe(Effect.map((version) => expect(version).toBe("2.1.234"))),
  );
