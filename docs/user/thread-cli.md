...
New scripted threads use the project's default provider and model selection. Override the model,
reasoning effort, or both for one invocation without changing the project default. Values are
checked against the selected provider instance's live model catalog before T3 creates the thread:

```sh
t3 thread run "Play a Forest Shuffle trial" --model gpt-6-astra --effort ultra
```

`--model` does not switch providers: it selects a model from the provider instance already chosen
by the project default. `--effort` likewise uses that model (or the model passed with `--model`) and
must be one of its advertised reasoning levels.

Scripted threads use the shared project checkout. The default permission mode matches new
interactive threads (`full-access`); select a different mode when the job should pause or constrain
changes:
