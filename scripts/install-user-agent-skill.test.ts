...
it.layer(NodeServices.layer)("install-user-agent-skill", (it) => {
  ...
  it.effect("keeps both updater integrations ordered before service replacement", () =>
    Effect.gen(function* () {
      ...
      const linuxCutoverPath = path.join(repoRoot, "scripts/complete-linux-update.sh");
      const macCutoverPath = path.join(repoRoot, "scripts/complete-mac-update.sh");

      const syntax = yield* Effect.sync(() =>
        NodeChildProcess.spawnSync(
          "bash",
          ["-n", installerPath, linuxPath, macPath, linuxCutoverPath, macCutoverPath],
          { encoding: "utf8" },
        ),
      );
      ...
      const integrations = [
        {
          path: linuxPath,
          handoffMarker: 'systemd-run --user --collect --property=Type=exec --unit="$handoff_unit"',
          cutoverPath: linuxCutoverPath,
          stopMarker: "systemctl --user stop t3code.service",
        },
        {
          path: macPath,
          handoffMarker: 'launchctl submit -l "$handoff_label"',
          cutoverPath: macCutoverPath,
          stopMarker: 'stop_launchd_service "$service_target"',
        },
      ];
      for (const integration of integrations) {
        const source = yield* fs.readFileString(integration.path);
        const cutover = yield* fs.readFileString(integration.cutoverPath);
        ...
        const handoffIndex = source.indexOf(integration.handoffMarker);

        assert.isAtLeast(shimIndex, 0);
        assert.isAbove(installerIndex, shimIndex);
        assert.isAbove(handoffIndex, installerIndex);
        ...
        if (integration.path === macPath) {
          assert.include(source, "$HOME/.local/share/mise/shims");
          assert.include(source, "$HOME/.vite-plus/bin");
          assert.include(source, "for tool in node vp git hdiutil ditto launchctl");
          assert.include(source, "verify_launchctl_handoff");
          assert.include(cutover, "wait_for_server_ready");
          assert.include(cutover, "write_status succeeded");
        }
        assert.include(cutover, 'scripts/prepare-desktop-update.ts" "$HOME/.t3"');
        assert.include(cutover, integration.stopMarker);
        assert.isBelow(
          cutover.indexOf("scripts/prepare-desktop-update.ts"),
          cutover.indexOf(integration.stopMarker),
        );
      }
    }),
  );
